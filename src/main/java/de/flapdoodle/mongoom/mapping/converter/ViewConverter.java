...
public class ViewConverter<T> extends AbstractReadOnlyConverter<T> implements IViewConverter<T> {
	...
	public ViewConverter(Mapper mapper, EntityConverter<T> entityConverter, MappingContext<T> context, Class<T> viewType) {
		...
		for (Field f : fields) {
			...
			if (converter == null)
				throw new MappingException(entityConverter.getEntityClass(), "View: " + getEntityClass()
						+ ", Could not get Attribute " + fieldName + " in Entity");

			//			converter.matchType(f.getType(),f.getGenericType());
			if (!converter.matchType(getEntityClass(), f.getType(), f.getGenericType()))
				throw new MappingException(entityConverter.getEntityClass(), "View: " + getEntityClass()
						+ ", converter does not match field type " + fieldName + ": " + converter + " ? " + f.toGenericString()
						+ ")");

			MappedAttribute mappedAttribute = new MappedAttribute(f, fieldName, converter);
			if (!_attributes.add(mappedAttribute)) {
				throw new MappingException(entityConverter.getEntityClass(), "View: " + getEntityClass() + ", Field with name "
						+ fieldName + " allready defined");
			}
