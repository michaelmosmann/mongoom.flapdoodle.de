...
//	@Override
	...
	{
		if (_not) throw new MappingException("not.not does not make any sense");
		_not=!_not;
		return this;
	}
	...
	{
		if (_not) throw new MappingException("use ne instead of not.eq");
		_queryBuilder.set(_field, getConverter(true).convertTo(value));
		return _query;
	}
...
//		_queryBuilder.set(_field, pattern);
	...
	{
		if (_not) throw new MappingException("use exists("+!exists+") instead");
		IDBObjectFactory factory = _queryBuilder.get(_field);
//		if (_not) factory=factory.get("$not");
...
//		_queryBuilder.set(_field, "$size", value);
	...
	public <V> Q type(Class<?> type)
	{
		...
		if (bsonType==null) throw new MappingException("Could not convert "+type+" to BSON Type");
		IDBObjectFactory factory = _queryBuilder.get(_field);
		if (_not) factory=factory.get("$not");
...
//		_queryBuilder.set(_field, "$type", bsonType);
	...
	{
		if (_not) throw new MappingException("use nin instead of not.in");
	...
	{
		if (_not) throw new MappingException("use in instead of not.nin");
	...
	{
		if (_not) throw new MappingException("use eq instead of not.ne");
	...
	{
		if (_not) throw new MappingException("use lte instead of not.gt");
	...
	{
		if (_not) throw new MappingException("use gte instead of not.lt");
	...
	{
		if (_not) throw new MappingException("use lt instead of not.gte");
	...
	{
		if (_not) throw new MappingException("use gt instead of not.lte");
	...
	public SubQuery<T,Q> elemMatch()
	{
		...
		{
			return new SubQuery<T,Q>(_query,((IContainerConverter) _converter).containerConverter(),_queryBuilder.get(_field).get("$elemMatch"));
		}
	...
	private <V> Q opList(String op, boolean listAllowed, V... value)
	{
		...
		for (V v : value)
		{
			values.add(getConverter(listAllowed).convertTo(v));
		}
		IDBObjectFactory factory = _queryBuilder.get(_field);
		if (_not) factory=factory.get("$not");
...
//		_queryBuilder.set(_field, op, values);
	...
	private <V> Q op(String op, boolean listAllowed, V value)
	{
		IDBObjectFactory factory = _queryBuilder.get(_field);
		if (_not) factory=factory.get("$not");
		factory.set(op, getConverter(listAllowed).convertTo(value));
