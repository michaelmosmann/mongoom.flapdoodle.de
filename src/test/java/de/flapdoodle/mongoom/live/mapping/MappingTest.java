...
//		datastore.save(d);
		...
		mq.setYear(2010);
		IEntityQuery<Document> query = datastore.with(Document.class).or()
			.field("meta").eq(mq)
			.field("meta.category").eq("fun")
			.field("name").in("Doc1","Doc2","Doc3").parent()
			.or()
			.field("name").eq("DocX")
			.parent()
			.or()
			.field("meta.year").not().type(Date.class)
			.parent()
			.or()
			.field("meta.tags").elemMatch().field("tag").eq("sommer").parent()
			.parent();
...
//		query.field("meta.tags").elemMatch().field("tag").eq("sommer");
		...
		List<UsernameEmailView> usernameEmails = datastore.with(User.class).withView(UsernameEmailView.class).order("email", false).asList();
